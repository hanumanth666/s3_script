...
def calculate_bucket_total_size(bucket_name, prefixes, lifecycle_policies, bucket_policy):
    ...
    for prefix in prefixes:
        ...
        for prefix, details in prefix_details.items():
            total_storage_size += details['TotalSize']

    return total_storage_size

def main(bucket_names):
    bucket_storage_sizes = {}
    for bucket_name in bucket_names:
        ...
        total_storage_size = calculate_bucket_total_size(bucket_name, prefixes, lifecycle_policies, bucket_policy)
        bucket_storage_sizes[bucket_name] = total_storage_size

    # Sort buckets by total storage size in descending order
    sorted_buckets = sorted(bucket_storage_sizes.items(), key=lambda x: x[1], reverse=True)

    # Print the list of buckets sorted by storage size
    print("\nBuckets sorted by total storage size (descending):")
    for bucket_name, total_size in sorted_buckets:
        print(f"{bucket_name}: {convert_size(total_size)}")

if __name__ == "__main__":
    bucket_names = ['bucket1', 'bucket2', 'bucket3']  # Replace with your S3 bucket names
    main(bucket_names)